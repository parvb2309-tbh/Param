...
export function wireArrowUpRecall(composer, getUserMessages, options = {}) {
  ...
  const debug = (...args) => {
    try {
      if (localStorage.getItem('paramArrowRecallDebug') === '1') {
  ...
  composer.addEventListener('input', () => {
    ...
    lastRecalledValue = '';
    recallHistory = [];
    try { delete composer.dataset.paramRecallIndex; } catch (_) {}
  });
  ...
  composer.addEventListener('keydown', (e) => {
    ...
    if (currentIndex < 0 && currentValue) {
      const markedIndex = Number(composer.dataset?.paramRecallIndex);
    ...
    if (e.key === 'ArrowDown') {
      ...
      try { composer.dataset.paramRecallIndex = String(nextIndex); } catch (_) {}
