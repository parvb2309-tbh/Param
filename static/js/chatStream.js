...
document.addEventListener('param:tool-approval', () => {
...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.param-highlight').forEach(function(e) { e.classList.remove('param-highlight'); });
      document.querySelectorAll('.param-hl-label').forEach(function(e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('param-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'param-hl-label';
