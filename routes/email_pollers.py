                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Param (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Param" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "param_kind" if "param_kind" in cols else "'scheduled' AS param_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                # externally cron-driven `param-mail poll-scheduled`, or
                # an admin running the CLI manually alongside the in-process
                # one despite the PARAM_INPROCESS_POLLERS=0 guidance) -
                ...
                param_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Param-Origin"] = "param-ui"
                outer["X-Param-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", param_kind or "scheduled")[:64]
                outer["X-Param-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `param-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `PARAM_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `param-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("PARAM_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `PARAM_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (PARAM_INPROCESS_POLLERS=0); "
            "drive `param-mail poll-scheduled` externally."
        )
        return
    import asyncio
