    ...
    script = f"""
      ...
      {{
        ...
        const res = {{ headers: {{ get(name) {{
          return name === 'X-Param-Run-Id' ? 'normal-run' : null;
        }} }} }};
        {header_capture}
      }}
      ...
      {{
        ...
        const res = {{ headers: {{ get(name) {{
          return name === 'X-Param-Run-Id' ? 'run-1' : null;
        }} }} }};
        {header_capture}
      }}
      await new Promise(resolve => setTimeout(resolve, 0));
      console.log(JSON.stringify({{
        ...
        calls: calls.map(call => ({{
          ...
          runId: call.options.headers['X-Param-Run-Id'],
        }})),
      }}));
    """
    ...
    script = f"""
      ...
      console.log(JSON.stringify({{
        ...
        afterHeaders: {{
          ...
          runId: calls[0] && calls[0].options.headers['X-Param-Run-Id'],
        }},
      }}));
    """
...
def test_resend_preserves_queued_stop_until_old_run_identity_arrives():
    ...
    script = f"""
      ...
      {{
        ...
        const res = {{ headers: {{ get(name) {{
          return name === 'X-Param-Run-Id' ? 'old-run' : null;
        }} }} }};
        {header_capture}
      }}
      await new Promise(resolve => setTimeout(resolve, 0));
    ...
    script = f"""
      ...
      console.log(JSON.stringify({{
        ...
        normal: {{
          ...
          stopRunId: calls[0] && calls[0].options.headers['X-Param-Run-Id'],
        }},
      }}));
    """
