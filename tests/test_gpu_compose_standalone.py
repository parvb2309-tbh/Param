...
base compose with only the matching overlay merged into the ``param``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on param only."""
...
# --- Non-param services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_param_services_match_base(base, standalone_path):
...
# --- param = base service + only the overlay additions ------------------


def test_nvidia_param_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_param_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
...
def test_base_has_no_host_docker_access(base):
    ...
    assert "PARAM_ENABLE_HOST_DOCKER=true" not in service["environment"]
