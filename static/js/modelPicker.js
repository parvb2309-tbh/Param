...
const RECENT_KEY = 'param-model-recent';
const FAVORITES_KEY = 'param-model-favorites';
const RECENT_MAX = 5;
...
function _initModelPickerDropdown() {
  ...
  async function _refreshLocalProbe() {
    try {
      if (window.__paramChatBusy || Date.now() < (window.__paramChatBusyUntil || 0)) return;
    } catch (_) {}
    const now = Date.now();
  ...
  const _collapsedProviders = new Set(_loadList('param-model-collapsed'));
    ...
    if (all.length <= BROWSE_ALL_LIMIT) {
      ...
      sorted.forEach(provider => {
        ...
        header.addEventListener('click', (e) => {
          ...
          _saveList('param-model-collapsed', [..._collapsedProviders]);
          const st = listEl.scrollTop;
          _populate('');
          listEl.scrollTop = st;
        });
...
async function _pick(m) {
    ...
    try { window.__paramModelSwitchPromise = switchPromise; } catch (_) {}
    const finishSwitch = () => {
      try {
        if (switchDone) switchDone();
        if (window.__paramModelSwitchPromise === switchPromise) delete window.__paramModelSwitchPromise;
      } catch (_) {}
    };
    ...
    try { document.dispatchEvent(new CustomEvent('param:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('param:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  if (!modelId && !currentSessionId && !_pendingChat && _deps.setPendingChat) {
    ...
    try {
      cachedDefault = window.__paramDefaultChat || null;
    } catch (_) {}
    if (!cachedDefault || !cachedDefault.endpoint_url || !cachedDefault.model) {
      try {
        cachedDefault = JSON.parse(localStorage.getItem('param-default-chat-cache') || 'null');
      } catch (_) {}
    }
    if (cachedDefault && cachedDefault.endpoint_url && cachedDefault.model) {
  ...
  // SECURITY: deliberately NOT auto-injecting `param-model-favorites[0]`
