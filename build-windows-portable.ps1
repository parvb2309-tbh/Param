...
<#
  Build a portable Windows distribution for Param.

  Output layout:
    dist\Param\Param.exe
    dist\Param\static\...
    dist\Param\scripts\...
    dist\Param\mcp_servers\...
    dist\Param\services\hwfit\data\...
...
& $pyExe -m PyInstaller --noconfirm --clean --onedir --noconsole --icon=static/icon.ico --name Param @dataArgs launcher.py
if ($LASTEXITCODE -ne 0) { Fail "PyInstaller build failed." }

Write-Host ""
Write-Host "Build complete." -ForegroundColor Green
Write-Host "Portable app folder: $PSScriptRoot\dist\Param" -ForegroundColor Green
