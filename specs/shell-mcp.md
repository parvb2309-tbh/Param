...
caller or configured through `PARAM_MCP_EMAIL_OWNER` /
`PARAM_EMAIL_OWNER`; if owner-scoped email accounts exist and no owner is
...
- Docker images include selected shell dependencies and the Docker CLI, but host Docker socket access from inside the app container remains unavailable unless the operator explicitly enables `docker/host-docker.yml`/`PARAM_ENABLE_HOST_DOCKER=true` and mounts a real socket.
