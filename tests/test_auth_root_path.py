...
def test_login_page_uses_mount_aware_urls():
    ...
    assert "window.__paramLoginAppUrl" in html
    for api_path in (
        "/api/version",
        "/api/auth/policy",
        "/api/auth/status",
    ...
    assert (
        "__paramLoginAppUrl || ((path) => path))('/static/js/theme.js')"
        in html
    )
...
def test_login_page_static_assets_resolve_under_mount_path():
    ...
    mounted_login_url = "https://example.test/param/login"
    for relative_asset, mounted_path in (
        ("static/manifest.json", "/param/static/manifest.json"),
        ("static/icons/icon-192.png", "/param/static/icons/icon-192.png"),
        (
            "static/fonts/FiraCode-Regular.woff2",
            "/param/static/fonts/FiraCode-Regular.woff2",
        ),
        (
            "static/fonts/FiraCode-SemiBold.woff2",
            "/param/static/fonts/FiraCode-SemiBold.woff2",
        ),
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/param", "/param/api/models", "/api/models"),
        ("/param/", "/param//api/models", "/api/models"),
        ("/", "//api/models", "/api/models"),
        ("/param", "/odyssey/api/models", "/odyssey/api/models"),
...
def test_real_auth_middleware_uses_application_relative_path(tmp_path):
    ...
    probe = textwrap.dedent(
        ...
        async def main():
            setup = await _case("/param", "/api/auth/setup", configured=False)
            mounted_api = await _case("/param", "/api/models", configured=True)
            mounted_browser = await _case("/param", "/notes", configured=True)
            webhook = await _case(
                "/param",
