...
Param keeps all of your state in the `data/` directory — the SQLite database
(`app.db`), the Fernet encryption key (`data/.app_key`), the vault, memory, RAG
indexes, personal documents, and uploads. The `scripts/param-backup` tool
...
```bash
# Create a snapshot → backups/param-backup-<YYYYMMDD-HHMMSS>.tar.gz
./scripts/param-backup snapshot

# List existing snapshots (most recent first)
./scripts/param-backup list

# Check a tarball's integrity without extracting it
./scripts/param-backup verify backups/param-backup-20260101-120000.tar.gz

# Restore (destructive — see the warning below)
./scripts/param-backup restore backups/param-backup-20260101-120000.tar.gz --yes
```
...
./scripts/param-backup snapshot --out /mnt/nas/param-$(date +%F).tar.gz

# Full snapshot including research runs and mail attachments
./scripts/param-backup snapshot --include-research --include-attachments
```
...
```cron
0 3 * * *  cd /path/to/param && ./scripts/param-backup snapshot --out "/mnt/nas/param-$(date +\%F).tar.gz"
```
...
> **not** under `./data`. `param-backup` therefore does not capture the Docker
