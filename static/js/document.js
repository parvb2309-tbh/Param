...
import { bindMenuDismiss, dismissOrRemove } from './escMenuStack.js';
  ...
  const _EMAIL_LOCAL_DRAFT_PREFIX = 'param.email.replyDraft.v1:';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__paramActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'param-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'param-doc-minimized-' + sessionId;
  ...
  let _paramAttachMenu = null;

  function _closeParamAttachMenu() {
    if (_paramAttachMenu) {
      _paramAttachMenu.remove();
      _paramAttachMenu = null;
    }
  ...
  function _attachMenuOutsideClick(e) {
    if (_paramAttachMenu && !_paramAttachMenu.contains(e.target)) _closeParamAttachMenu();
  }

  function _attachMenuEscape(e) {
    if (e.key !== 'Escape') return;
    _closeParamAttachMenu();
  }

  function _positionParamAttachMenu(anchor, menu) {
  ...
  function _paramAttachLabel(item, kind) {
    if (kind === 'gallery') {
  ...
  async function _stageParamAttachment(kind, id) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-param`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function _stageParamZip(items) {
    const doc = docs.get(activeDocId);
    if (!doc || doc.language !== 'email') return null;
    if (!doc._composeAtts) doc._composeAtts = [];
    const res = await fetch(`${API_BASE}/api/email/compose-from-param-zip`, {
      method: 'POST',
      credentials: 'same-origin',
      headers: { 'Content-Type': 'application/json' },
  ...
  function _afterParamAttachmentsAdded(count, label) {
    _renderComposeAttachments();
    clearTimeout(_autoSaveDebounce);
    _autoSaveDebounce = setTimeout(() => { saveDocument({ silent: true }); }, 800);
  ...
  async function _attachParamItem(kind, id, label, opts = {}) {
    try {
      const data = await _stageParamAttachment(kind, id);
      if (!data) return;
      _afterParamAttachmentsAdded(1, label || data.filename);
      if (!opts.keepOpen) _closeParamAttachMenu();
    } catch (err) {
      console.error('Failed to attach Param item:', err);
      if (uiModule) uiModule.showError('Failed to attach from Param');
    }
  }

  function _selectedParamAttachRows(menu) {
    return Array.from(menu?.querySelectorAll?.('.email-param-attach-row.is-selected') || []);
  }

  function _syncParamAttachSelection(menu) {
    const selected = _selectedParamAttachRows(menu);
    const bar = menu?.querySelector?.('.email-param-attach-actions');
    const count = menu?.querySelector?.('.email-param-attach-count');
    const attachBtn = menu?.querySelector?.('.email-param-attach-selected');
  ...
  async function _attachSelectedParamItems(menu) {
    const rows = _selectedParamAttachRows(menu);
    if (!rows.length) return;
    const btn = menu.querySelector('.email-param-attach-selected');
    if (btn) {
      btn.disabled = true;
    ...
    try {
      ...
      if (zip) {
        await _stageParamZip(items);
        added = 1;
      } else {
        for (const item of items) {
          await _stageParamAttachment(item.kind, item.id);
          added += 1;
        }
      }
      _afterParamAttachmentsAdded(added, zip ? 'param-attachments.zip' : undefined);
      _closeParamAttachMenu();
    } catch (err) {
      console.error('Failed to attach selected Param items:', err);
      if (uiModule) uiModule.showError(added ? `Attached ${added}, then failed` : 'Failed to attach from Param');
      _renderComposeAttachments();
    } finally {
      if (btn) {
  ...
  async function _loadParamAttachItems(menu, kind) {
    const list = menu.querySelector('.email-param-attach-list');
    if (!list) return;
    ...
    const q = (menu.querySelector('.email-param-attach-search')?.value || '').trim();
    try {
      ...
      if (!items.length) {
        list.innerHTML = `<div class="email-param-attach-empty">${q ? 'No matches' : `No ${kind === 'gallery' ? 'images' : 'documents'}`}</div>`;
        _syncParamAttachSelection(menu);
        return;
      }
      list.innerHTML = '';
      for (const item of items) {
        const label = _paramAttachLabel(item, kind);
        const row = document.createElement('button');
        row.type = 'button';
        row.className = `email-param-attach-row ${kind === 'gallery' ? 'is-gallery' : ''}`;
        row.dataset.id = item.id || '';
        row.dataset.kind = kind;
        if (kind === 'gallery') {
          const src = item.url ? `${API_BASE}${item.url}` : '';
          row.innerHTML = `
            <span class="email-param-attach-dot" aria-hidden="true"></span>
            <span class="email-param-attach-thumb">${src ? `<img src="${_escHtml(src)}" alt="">` : ''}</span>
            <span class="email-param-attach-main">
              <span class="email-param-attach-title">${_escHtml(label)}</span>
              <span class="email-param-attach-meta">${_escHtml(item.filename || 'image')}</span>
            </span>
          `;
        } else {
          row.innerHTML = `
            <span class="email-param-attach-dot" aria-hidden="true"></span>
            <span class="email-param-attach-icon">${langIcon(item.language || 'text', 14, { style: 'opacity:0.8;' })}</span>
            <span class="email-param-attach-main">
              <span class="email-param-attach-title">${_escHtml(label)}</span>
              <span class="email-param-attach-meta">${_escHtml(item.language || 'text')}</span>
            </span>
          `;
        }
        ...
        row.addEventListener('dblclick', () => _attachParamItem(kind, item.id, label, { keepOpen: false }));
        list.appendChild(row);
      }
      _syncParamAttachSelection(menu);
    } catch (err) {
      console.error('Failed to load Param attach items:', err);
      list.innerHTML = '<div class="email-param-attach-empty">Could not load</div>';
    }
  }
  ...
  function _showComposeAttachMenu(anchor) {
    ...
    menu.className = 'email-param-attach-menu';
    menu.innerHTML = `
      <button type="button" class="email-param-attach-local">
      ...
      <div class="email-param-attach-tabs">
      ...
      <label class="email-param-attach-search-wrap">
        ...
        <input type="search" class="email-param-attach-search" placeholder="Search attachments">
      </label>
      <div class="email-param-attach-list"></div>
      <div class="email-param-attach-actions">
        <span class="email-param-attach-count"></span>
        <button type="button" class="email-param-attach-selected" disabled>
    ...
    _paramAttachMenu = menu;
    _positionParamAttachMenu(anchor, menu);
    menu.querySelector('.email-param-attach-local')?.addEventListener('click', () => {
      _closeParamAttachMenu();
      document.getElementById('doc-email-file-input')?.click();
    });
    menu.querySelectorAll('[data-ody-attach-kind]').forEach(btn => {
      btn.addEventListener('click', () => _loadParamAttachItems(menu, btn.dataset.odyAttachKind));
    });
    let attachSearchTimer = null;
    menu.querySelector('.email-param-attach-search')?.addEventListener('input', () => {
      clearTimeout(attachSearchTimer);
      attachSearchTimer = setTimeout(() => {
        _loadParamAttachItems(menu, menu.dataset.odyAttachKind || 'document');
      }, 220);
    });
    menu.querySelector('.email-param-attach-selected')?.addEventListener('click', () => _attachSelectedParamItems(menu));
    setTimeout(() => {
      document.addEventListener('click', _attachMenuOutsideClick, true);
      document.addEventListener('keydown', _attachMenuEscape, true);
    }, 0);
    _loadParamAttachItems(menu, 'document');
  }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__paramActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  const _AI_REPLY_CONTEXT_STORE_PREFIX = 'param:email-ai-reply-context:v1:';
  ...
  async function _aiReply(opts = {}) {
    ...
    const sourceUid = document.getElementById('doc-email-source-uid')?.value?.trim() || '';
    const sourceFolder = document.getElementById('doc-email-source-folder')?.value?.trim() || 'INBOX';
    const sourceAccountId = docs.get(activeDocId)?.sourceEmailAccountId || window.__paramActiveEmailAccount || '';
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div id="doc-email-actions" class="doc-email-actions" style="display:none">
        ...
        <div class="email-send-split">
          <button type="button" id="doc-email-send-btn" class="email-send-btn email-send-main" title="Send email (Ctrl+Enter)" onpointerdown="window.paramEmailSendIntent&&window.paramEmailSendIntent(event)" onmousedown="window.paramEmailSendIntent&&window.paramEmailSendIntent(event)" onclick="window.paramEmailSendIntent&&window.paramEmailSendIntent(event)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>Send</button>
          <button type="button" id="doc-email-send-caret" class="email-send-btn email-send-caret" title="More send options" aria-haspopup="true" aria-expanded="false" onpointerdown="window.paramEmailCaretIntent&&window.paramEmailCaretIntent(event)" onmousedown="window.paramEmailCaretIntent&&window.paramEmailCaretIntent(event)"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 9 12 15 18 9"/></svg></button>
    ...
    const handleSendIntent = (e) => {
      if (e && e.__paramEmailSendHandled) return;
      ...
      if (e) {
        e.preventDefault();
        e.stopPropagation();
        e.__paramEmailSendHandled = true;
      }
    ...
    window.paramEmailSendIntent = handleSendIntent;
    ...
    const handleCaretIntent = (e) => {
      if (e && e.__paramEmailCaretHandled) return;
      const now = Date.now();
      if (e && e.type === 'click' && now - lastCaretToggleAt < 350) {
        e.preventDefault();
        e.stopPropagation();
        e.__paramEmailCaretHandled = true;
        return;
      }
      const rawTarget = e && e.target;
      const target = rawTarget && rawTarget.nodeType === Node.TEXT_NODE ? rawTarget.parentElement : rawTarget;
    ...
    window.paramEmailCaretIntent = handleCaretIntent;
    ...
    let _fontIdx = parseInt(localStorage.getItem('param-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('param-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'param-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('param-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('param-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('param-suggestions-' + docId);
      if (!raw) return;
