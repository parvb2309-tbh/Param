...
if command -v nvidia-smi >/dev/null 2>&1; then
  ...
  if [ -n "$alive" ]; then
    echo "[param] Force killing remaining GPU PIDs:$alive"
  ...
  if [ -n "$remaining" ]; then
    echo "[param] GPU processes still remain:"
    echo "$remaining"
    exit 2
  fi
  echo "[param] GPU cleanup complete. No NVIDIA compute processes remain."
else
  echo "[param] nvidia-smi not found; falling back to common model-server process cleanup."
  ...
  echo "[param] Fallback cleanup complete."
fi`;
}
...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    fixes: [
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('vllm') },
      {
        ...
        action: () => _copyText('Upgrade the vLLM environment that provides the selected vllm CLI, or use a compatible checkpoint. Do not assume Param owns PATH/system/source/Docker installs.'),
      },
    ],
  },
  {
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch with Param’ venv CUDA library path fix. If the venv does not contain the matching NVIDIA runtime libs, run Repair sglang-kernel.',
    fixes: [
  ...
  },
  {
    ...
    suggestion: 'Suggested action: relaunch from the cached local snapshot path. Param now rewrites MLX repo-id launches to the newest local Hugging Face snapshot when it exists on the selected Mac.',
    fixes: [
      { label: 'Edit / relaunch serve', action: (panel) => _openServeEditFromDiagnosis(panel) },
      { label: 'Open Dependencies', action: () => _openCookbookDependencies('mlx_lm') },
      { label: 'Copy error', action: (panel) => {
...
function _diagnosisCopyBundle(task, diagnosis, sourceText, suggestionText) {
  const lines = ['## Param Cookbook troubleshooting'];
  if (task) {
    lines.push(
      '',
      '### Task',
