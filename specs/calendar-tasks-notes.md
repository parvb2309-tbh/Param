...
- direct DB CLIs `scripts/param-calendar`, `scripts/param-notes`, and `scripts/param-tasks`;
...
- route-level no-login calendar access normalizes empty owner values to `PARAM_FALLBACK_OWNER` or `owner@localhost`, so route-created calendar rows do not use the empty string as their storage owner;
...
- the in-process scheduler is gated by `PARAM_INPROCESS_TASKS`, and multiple enabled app processes can double-run work.
...
- `scripts/param-webhook` builds the live `/api/tasks/{task_id}/webhook/{token}` path with percent-encoded path segments; its direct DB token rotation/revocation behavior remains a local compatibility surface.
