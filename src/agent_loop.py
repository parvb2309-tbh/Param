...
Streaming agent loop for param-ui.
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Param chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
```
GENERIC LOOPBACK to allowed Param internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _local_computer_rules() -> str:
    return (
        "\n\n## Param Terminus local-machine mode\n"
...
def _minimal_saved_memory_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    if not facts:
        return None
    logger.info("[agent-intent] param doc minimal memory facts=%s", len(facts))
    return untrusted_context_message(
        "saved memory: minimal context",
        (
            "Saved user memory facts from Param Brain. These are the same "
...
def _minimal_recent_notes_tool_context_message(messages: List[Dict]) -> Optional[Dict]:
    ...
    return untrusted_context_message(
        "recent tool context",
        (
            "Recent Param tool context for follow-up references only. "
...
def _minimal_param_doc_messages(messages: List[Dict], active_document, stream_create: bool = False) -> List[Dict]:
    """Tiny prompt path for the Param document LoRA.
    ...
    if stream_create:
        system = (
            "You are Param. Create the requested document by streaming exactly one fenced block:\n"
            "```document\n"
            "Title\n"
            "markdown\n"
            "Document content\n"
        ...
        system = (
            "You are Param. Edit or suggest changes to the active document using exactly one fenced tool block when needed.\n"
            ...
            "After the tool succeeds, Param will answer Done."
        )
...
def _minimal_param_notes_messages(messages: List[Dict]) -> List[Dict]:
    """Tiny prompt path for Param notes/calendar/tasks LoRAs.

    The finetune is trained to emit Param notes/calendar/task tool calls
    without receiving the full tool schema or saved-context wrapper stack.
    """
    latest = _extract_last_user_message(messages)
    system = (
        "You are Param. Handle notes, reminders, calendar events, and scheduled tasks.\n"
...
def _minimal_param_general_messages(messages: List[Dict], include_memory: bool = False) -> List[Dict]:
    """Minimal fallback for Param finetunes outside domain-specific paths."""
    latest = _extract_last_user_message(messages)
    system = (
        "You are Param. Answer directly and briefly.\n"
        "Use Param tool-call format only when the user explicitly asks you to take an action.\n"
...
def _normalize_ody_qwen_text_artifacts(text: str) -> str:
    """Repair common dropped-final-letter artifacts from small Param LoRAs.

    This is intentionally scoped to the param-qwen3 runtime path. It is not
...
def _is_param_qwen_model(model: str) -> bool:
    return (model or "").lower().startswith("param-qwen3")


def _ody_qwen_temperature_cap(temperature):
    """Force-cap param-qwen3 sampling; the finetune destabilizes above 0.2.
    ...
    if active_document:
        ...
        if _is_email_doc:
            ...
            doc_ctx = (
                ...
                f'2. Replace ONLY the new reply text above `---------- Previous message ----------`. You may omit the quoted history from your tool output; Param preserves everything from that separator downward automatically.\n'
...
def _compute_final_metrics(
    ...
    # Context % should describe the prompt Param assembled, not provider
...
async def stream_agent_loop(
    ...
    _ody_qwen_finetune_model = _is_param_qwen_model(model)
    ...
    if _direct_low_signal:
        ...
        direct_messages = (
            _minimal_param_general_messages(
                messages,
                include_memory=True,
            )
            if _ody_qwen_finetune_model
        ...
        def _direct_candidate_request(_index, _url, candidate_model, _headers):
            candidate_is_qwen = _is_param_qwen_model(candidate_model)
            candidate_messages = (
                _minimal_param_general_messages(messages, include_memory=True)
    ...
    if not guide_only and _relevant_tools is not None:
        ...
        if (
            ...
            logger.info("[tool-rag] Workspace file/terminal request; using Param Terminus toolset")
    ...
    def _route_finetune_modes(candidate_model: str):
        is_ody = _is_param_qwen_model(candidate_model)
        doc_mode = (
            is_ody
    ...
    if _ody_doc_finetune_mode and _relevant_tools is not None:
        logger.info("[agent-intent] param doc finetune tool clamp=%s", sorted(_relevant_tools))
    elif _ody_notes_finetune_mode and _relevant_tools is not None:
        disabled_tools.difference_update({
            "manage_notes", "manage_calendar", "manage_tasks",
        })
        logger.info("[agent-intent] param notes finetune tool clamp=%s", sorted(_relevant_tools))
        ...
        try:
            ...
            pass
        logger.info("[agent-intent] param general no-tool clamp active")
    ...
    async def _build_route_request_state(candidate_url, candidate_model, candidate_headers, source_messages):
        ...
        if doc_mode and not plan_mode and not approved_plan and not guide_only:
            route_messages = _minimal_param_doc_messages(
                route_messages,
                _prompt_active_document,
            ...
            route_messages = _minimal_param_notes_messages(route_messages)
            route_mcp_schemas = []
        elif (
            is_ody
            and not _runtime_skill_tools
            ...
            route_messages = _minimal_param_general_messages(route_messages, include_memory=True)
            route_mcp_schemas = []
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        async def _candidate_request(index, candidate_url, candidate_model, candidate_headers):
            ...
            return {
                ...
                "kwargs": {
                    ...
                    "temperature": (
                        _ody_qwen_temperature_cap(_requested_temperature)
                        if _is_param_qwen_model(candidate_model)
                        else _requested_temperature
                    ),
                },
            }
        ...
        if _ody_doc_stream_create_mode and tool_blocks:
            ...
            if create_idx is None:
                logger.info(
                    "[agent] param doc stream-create discarded non-create tool call(s): %s",
                ...
                if len(tool_blocks) > 1 or create_idx != 0:
                    logger.info(
                        "[agent] param doc stream-create keeping first create_document and dropping extras: %s",
                        [block.tool_type for block in tool_blocks],
                    )
        ...
        if _ody_qwen_finetune_model and tool_blocks:
            ...
            if _dropped_memory_lookup:
                logger.info(
                    "[agent-intent] param qwen dropped manage_memory lookup; answering from compact memory"
                )
        ...
        if _doc_stream_create_completed:
            ...
            logger.info("[agent] param doc stream-create completed after one create_document")
            break
        ...
        if _ody_doc_tool_completed:
            if not full_response.strip() or full_response.strip().startswith("```"):
                full_response = "Done."
                yield 'data: ' + json.dumps({"delta": "Done."}) + '\n\n'
            logger.info("[agent] param doc tool completed after one textual tool block")
            break

        if (_ody_notes_finetune_mode or _ody_qwen_finetune_model) and _ody_notes_tool_completed:
            logger.info("[agent] param completed from deterministic tool output")
            break
