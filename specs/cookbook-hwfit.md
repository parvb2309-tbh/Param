...
- helper/CLI scripts `scripts/param-cookbook`, `scripts/add_hwfit_models.py`, `scripts/hf_download.py`, and `scripts/diffusion_server.py`;
...
- the `param-cookbook` CLI using the same Cookbook state file.
...
- local Docker inside the Param container is available only when the Docker CLI exists, `PARAM_ENABLE_HOST_DOCKER=true`, and `/var/run/docker.sock` is actually mounted as a socket; otherwise Cookbook should show the host-Docker access hint and prefer remote SSH Docker workflows;
...
Cookbook state lives under the shared data dir through the `COOKBOOK_STATE_FILE` constant, normally `data/cookbook_state.json`. Routes and the `param-cookbook` CLI use the same state path.
