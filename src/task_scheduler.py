...
class TaskScheduler:
        ...
        try:
            ...
            if gate_foreground:
                ...
                if waiting and waiting.status == "queued":
                    waiting.result = "Queued — waiting for Param to be idle…"
                    db.commit()
                ...
                async def _cancel_if_foreground_active():
                    ...
                    while True:
                        ...
                        if has_foreground_activity():
                            ...
                            logger.info("Task '%s' interrupted because Param became active", task.name)
                            if current_task:
                                current_task.cancel()
                            return
            ...
            try:
                ...
                msg = (
                    "Paused because Param became active"
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-Param-Origin"] = "param-ui"
            msg["X-Param-Kind"] = "task"
            msg["X-Param-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-Param-Origin": "param-ui",
                "X-Param-Kind": "task",
                "X-Param-Ref": str(task.id),
            },
        }
        if recipient:
    ...
    async def stop_background_tasks_for_foreground(self, *, reason: str = "Param became active") -> int:
        ...
        user opens or uses Param, foreground interaction wins immediately.
