...
def test_redirect_base_defaults_to_the_bound_port(monkeypatch):
    ...
    # callback pinned to 7000 lands on AirPlay instead of Param.
...
def test_redirect_base_prefers_the_explicit_origin(monkeypatch):
    ...
    resolved = _resolve_base(
        monkeypatch, OAUTH_REDIRECT_BASE_URL="https://param.example/", APP_PORT="7860"
    )
    assert resolved == "https://param.example"
...
def _authorize_page():
    ...
    return _oauth_authorize_page(
        ...
        "srv-1",
        "https://param.example.com/api/mcp/oauth/callback",
    )
...
def test_redirect_base_override_is_forwarded_into_the_container():
    ...
    for name in _COMPOSE_FILES:
        ...
        environment = set(compose["services"]["param"]["environment"])
