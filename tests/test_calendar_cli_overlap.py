"""Regression: `param-calendar list` must select events that OVERLAP the
...
`param-calendar list` with the default start=now()).
"""
...
def _load_cli(monkeypatch, rows):
    ...
    path = ROOT / "scripts" / "param-calendar"
    loader = importlib.machinery.SourceFileLoader("param_calendar_cli", str(path))
