...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Param runs from a virtualenv, that bin dir holds the tools the
...
def _venv_safe_local_pip_install_cmd(cmd: str, *, local: bool, in_venv: bool) -> str:
    ...
    prepends Param' own interpreter directory to PATH. If Param itself is
...
def _append_pip_install_runner_lines(runner_lines: list[str], cmd: str) -> None:
    ...
    runner_lines.append('  echo "[param] pip does not support --break-system-packages; installing without it."')
...
def _user_shell_path_bootstrap() -> list[str]:
    return [
        'PARAM_USER_SHELL="${SHELL:-}"',
        'if [ -n "$PARAM_USER_SHELL" ] && [ -x "$PARAM_USER_SHELL" ]; then',
        '  PARAM_USER_PATH="$("$PARAM_USER_SHELL" -ic \'printf "__PARAM_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__PARAM_PATH__//p\' | tail -n 1 || true)"',
        '  if [ -n "$PARAM_USER_PATH" ]; then export PATH="$PARAM_USER_PATH:$PATH"; fi',
        'fi',
...
def _cached_model_scan_script(model_dirs: list[str] | None = None, add_hf_cache: str | None = None) -> str:
    ...
    lines = [
        ...
        "    if os.name == 'nt' and not os.environ.get('PARAM_ALLOW_OLLAMA_CLI_SCAN'): return",
...
def _ollama_bind_from_cmd(cmd: str | None, *, default_host: str = "127.0.0.1") -> tuple[str, str]:
    ...
    wider default host so the resulting API is reachable by Param.
    """
    if not cmd:
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$PARAM_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $PARAM_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        ...
        runner_lines.append('  exit "$PARAM_PREFLIGHT_EXIT"')
    runner_lines.append('fi')
...
def _append_vllm_linux_preflight_lines(runner_lines: list[str]) -> None:
    ...
    # Keep the user install bin visible for Param-managed `pip install --user`
    # installs, but then report the actual CLI path so external runtimes are clear.
    runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
    runner_lines.append('PARAM_VLLM_BIN="$(command -v vllm 2>/dev/null || true)"')
    runner_lines.append('if [ -z "$PARAM_VLLM_BIN" ]; then')
    runner_lines.append('  echo "ERROR: vLLM is not installed."')
    runner_lines.append('  PARAM_PREFLIGHT_EXIT=127')
    runner_lines.append('else')
    runner_lines.append('  echo "[param] vLLM CLI: $PARAM_VLLM_BIN"')
    runner_lines.append('  PARAM_VLLM_VERSION="$("$PARAM_VLLM_BIN" --version 2>&1 | head -n 1 || true)"')
    runner_lines.append('  if [ -n "$PARAM_VLLM_VERSION" ]; then echo "[param] vLLM version: $PARAM_VLLM_VERSION"; fi')
    runner_lines.append('fi')
...
def _append_serve_exit_code_lines(
    ...
    *,
    ...
    runner_lines.append('PARAM_CMD_EXIT=$?')
    if is_pip_install:
        runner_lines.append('if [ $PARAM_CMD_EXIT -eq 0 ]; then echo ""; echo "DOWNLOAD_OK"; fi')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $PARAM_CMD_EXIT ==="')
        ...
        runner_lines.append('echo ""; echo "=== Process exited with code $PARAM_CMD_EXIT ==="')
        runner_lines.append('exit "$PARAM_CMD_EXIT"')
...
def _append_llama_cpp_linux_accel_build_lines(runner_lines: list[str]) -> None:
    ...
    # every OS-package dep from the launch path. Sets _param_have_prebuilt=1
    ...
    runner_lines.append('    _param_have_prebuilt=""')
    runner_lines.append('    _param_arch="$(uname -m)"')
    runner_lines.append('    _param_prebuilt_url=""')
    runner_lines.append('    if command -v curl >/dev/null 2>&1 && [ "$_param_arch" = "x86_64" ]; then')
    runner_lines.append('      _param_pat=""')
    runner_lines.append('      _param_has_nv_inline() { command -v nvidia-smi >/dev/null 2>&1 && nvidia-smi -L 2>/dev/null | grep -q "GPU "; }')
    runner_lines.append('      _param_has_vk_inline() { ldconfig -p 2>/dev/null | grep -q "libvulkan\\.so" || command -v vulkaninfo >/dev/null 2>&1 || [ -e /usr/lib/x86_64-linux-gnu/libvulkan.so.1 ]; }')
    runner_lines.append('      _param_has_vkdev_inline() { ls /dev/dri/renderD* >/dev/null 2>&1 || (lspci 2>/dev/null | grep -Ei \'VGA|3D|Display\' | grep -Eiq \'AMD|ATI|Radeon\'); }')
    runner_lines.append('      if _param_has_nv_inline; then')
    runner_lines.append('        _param_pat="ubuntu.*cuda"')
    runner_lines.append('      elif _param_has_vkdev_inline && _param_has_vk_inline; then')
    runner_lines.append('        _param_pat="ubuntu.*vulkan"')
    runner_lines.append('      else')
    runner_lines.append('        _param_pat="ubuntu-x64\\\\.zip"')
    runner_lines.append('      fi')
    runner_lines.append('      _param_prebuilt_url="$(curl -fsSL --max-time 15 https://api.github.com/repos/ggml-org/llama.cpp/releases/latest 2>/dev/null | grep \'"browser_download_url"\' | cut -d\'"\' -f4 | grep -iE "$_param_pat" | grep -iv "arm\\|aarch64" | head -1)"')
    runner_lines.append('    fi')
    ...
    runner_lines.append('    if [ -n "$_param_prebuilt_url" ] && (command -v unzip >/dev/null 2>&1 || command -v bsdtar >/dev/null 2>&1 || command -v python3 >/dev/null 2>&1); then')
    runner_lines.append('      echo "[param] Found prebuilt llama-server: $_param_prebuilt_url"')
    runner_lines.append('      mkdir -p ~/bin "$HOME/.cache/param/llama-cpp-prebuilt" && cd "$HOME/.cache/param/llama-cpp-prebuilt"')
    runner_lines.append('      rm -f llama-cpp.zip')
    runner_lines.append('      if curl -fsSL --max-time 120 "$_param_prebuilt_url" -o llama-cpp.zip && [ -s llama-cpp.zip ]; then')
    ...
    runner_lines.append('        _param_extracted="$(find build -type f -name llama-server 2>/dev/null | head -1)"')
    runner_lines.append('        if [ -n "$_param_extracted" ]; then')
    runner_lines.append('          chmod +x "$_param_extracted"')
    runner_lines.append('          ln -sf "$_param_extracted" ~/bin/llama-server')
    runner_lines.append('          _param_libdir="$(dirname "$_param_extracted")"')
    runner_lines.append('          mkdir -p ~/.config && echo "export LD_LIBRARY_PATH=\\"$_param_libdir:\\${LD_LIBRARY_PATH:-}\\"" > ~/.config/param-llama-cpp-env')
    runner_lines.append('          _param_have_prebuilt=1')
    runner_lines.append('          echo "[param] Prebuilt llama-server installed at $_param_extracted"')
    runner_lines.append('        fi')
    runner_lines.append('      fi')
    runner_lines.append('      [ -z "$_param_have_prebuilt" ] && echo "[param] Prebuilt download/extract failed — falling back to from-source build."')
    runner_lines.append('    elif [ -z "$_param_prebuilt_url" ]; then')
    runner_lines.append('      echo "[param] No matching prebuilt llama-server for this host (arch=$_param_arch) — will build from source."')
    runner_lines.append('    fi')
    runner_lines.append('  if [ -z "$_param_have_prebuilt" ]; then')
    ...
    runner_lines.append('    _param_has_nvidia_hw() {')
    ...
    runner_lines.append('    }')
    runner_lines.append('    if _param_has_nvidia_hw; then')
    ...
    runner_lines.append('    _param_apt_bootstrap() {')
    ...
    runner_lines.append('        echo "[param] Auto-installing missing build deps via apt:$_missing"')
    ...
    runner_lines.append('        echo "[param] Auto-installing missing build deps via pacman:$_missing"')
    ...
    runner_lines.append('        echo "[param] Auto-installing missing build deps via dnf:$_missing"')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[param] WARNING: missing build deps ($_missing) — passwordless sudo is unavailable, cannot auto-install. Cookbook Diagnosis will explain the fix after the build fails."')
    runner_lines.append('      fi')
    runner_lines.append('    }')
    runner_lines.append('    _param_apt_bootstrap')
    runner_lines.append('    _param_missing_build_deps=""')
    runner_lines.append('    command -v cmake >/dev/null 2>&1 || _param_missing_build_deps="$_param_missing_build_deps cmake"')
    runner_lines.append('    command -v git >/dev/null 2>&1 || _param_missing_build_deps="$_param_missing_build_deps git"')
    runner_lines.append('    command -v g++ >/dev/null 2>&1 || command -v gcc >/dev/null 2>&1 || _param_missing_build_deps="$_param_missing_build_deps build-essential"')
    runner_lines.append('    if [ -n "$_param_missing_build_deps" ]; then')
    runner_lines.append('      echo "ERROR: llama.cpp source build needs missing packages:$_param_missing_build_deps"')
    ...
    runner_lines.append('    _param_has_vulkan() {')
    ...
    runner_lines.append('      return 1')
    runner_lines.append('    }')
    runner_lines.append('    _param_has_vulkan_device() {')
    ...
    runner_lines.append('      fi')
    runner_lines.append('      echo "[param] ROCm/HIP detected — building llama-server with HIP support..."')
    ...
    runner_lines.append('    elif command -v nvcc &>/dev/null && _param_has_nvidia_hw; then')
    runner_lines.append('      rm -rf build')
    ...
    runner_lines.append('      _param_has_cudart() {')
    ...
    runner_lines.append('        return 1')
    runner_lines.append('      }')
    runner_lines.append('      if _param_has_cudart; then')
    runner_lines.append('        echo "[param] CUDA nvcc + cudart found — building llama-server with CUDA (GPU) support..."')
    ...
    runner_lines.append('      else')
    runner_lines.append('        echo "[param] WARNING: nvcc found but CUDA runtime (libcudart.so) is not visible — building llama-server for CPU only."')
    runner_lines.append('        echo "[param]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('        echo "[param]   Ensure libcudart is installed (e.g. cuda-runtime package) and visible via ldconfig or CUDA_HOME."')
    ...
    runner_lines.append('      fi')
    runner_lines.append('    elif _param_has_vulkan_device && _param_has_vulkan; then')
    runner_lines.append('      echo "[param] Vulkan-capable GPU detected (no ROCm/CUDA toolchain installed) — building llama-server with Vulkan support..."')
    ...
    runner_lines.append('    else')
    runner_lines.append('      echo "[param] WARNING: no HIP/CUDA/Vulkan toolchain found — building llama-server for CPU only."')
    runner_lines.append('      echo "[param]   GPU inference will not be available for this llama.cpp build."')
    runner_lines.append('      echo "[param]   Install Vulkan (libvulkan-dev) / ROCm for AMD GPUs or CUDA tooling for NVIDIA, then re-launch this serve task."')
    runner_lines.append('      rm -rf build')
    runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release && cmake --build build -j"$NPROC" --target llama-server && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
    runner_lines.append('    fi')
    runner_lines.append('  fi  # end _param_have_prebuilt guard')
...
def _llama_cpp_rebuild_cmd(update_source: bool = False) -> str:
    ...
    if update_source:
        update_cmd = (
            ...
            'echo "[param] WARNING: llama.cpp source update failed; clearing cached build anyway."; '
            'elif command -v git >/dev/null 2>&1; then '
            'git clone --depth 1 https://github.com/ggml-org/llama.cpp "$HOME/llama.cpp" || '
            'echo "[param] WARNING: llama.cpp clone failed; clearing cached build anyway."; '
            'fi && '
        )
    return (
        'mkdir -p "$HOME/bin" && '
        ...
        'echo "[param] Cleared the cached llama.cpp build. '
...
WIN_SESSION_DIR = "$env:TEMP\\\\param-sessions"
...
def _diagnose_serve_output(text: str) -> dict | None:
    ...
    patterns = [
        ...
        (
            r"mlx-lama-swift|param-mlx-inpaint|mlx-lama-serve|LaMa / MI-GAN MLX inpainting models require",
            "LaMa / MI-GAN MLX inpainting requires an Param-compatible mlx-lama-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-lama-swift bridge and put param-mlx-inpaint or mlx-lama-serve on PATH", "op": "dependency", "package": "mlx_lama_swift"}],
        ),
        (
            r"mlx-ddcolor-swift|param-mlx-colorize|mlx-ddcolor-serve|DDColor MLX models require",
            "DDColor MLX colorization requires an Param-compatible mlx-ddcolor-swift bridge on this Apple Silicon server.",
            [{"label": "build mlx-ddcolor-swift bridge and put param-mlx-colorize or mlx-ddcolor-serve on PATH", "op": "dependency", "package": "mlx_ddcolor_swift"}],
        ),
        (
