...
def test_param_notes_mode_clamps_and_reenables_all_personal_managers(monkeypatch):
...
def test_param_general_mode_disables_every_tool(monkeypatch):
...
def test_param_calendar_intent_uses_notes_mode(monkeypatch):
    prompt_calls, _ = _install_route_probe(monkeypatch)
...
def test_param_calendar_followup_keeps_notes_mode(monkeypatch):
    prompt_calls, _ = _install_route_probe(monkeypatch)
    messages = [
        {"role": "user", "content": "Add lunch tomorrow to my calendar."},
        {
...
def test_param_qwen_temperature_is_capped_for_agent_requests(monkeypatch):
